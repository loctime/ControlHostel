...
export function LandingEditor({ hostelId, slug, initialConfig, onClose, onSaved }: Props) { 
  ...
  function moveBlock(id: string, dir: "up" | "down") { 
    setBloques((prev) => { 
      ...
      const newSorted = sorted.map((b, i) => { 
  ...
  return ( 
    <> 
      ...
      {/* Panel lateral */} 
      <div 
        className="fixed right-0 top-0 z-50 flex h-screen w-full max-w-md flex-col bg-[#1a1f36] shadow-2xl" 
        style={{ borderLeft: "1px solid rgba(124,131,255,0.2)" }} 
      > 
        {/* Header */} 
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4"> 
          <span className="font-semibold text-white">Editor de landing</span> 
          <div className="flex items-center gap-3"> 
            <button 
              onClick={() => void onSave()} 
              disabled={busy} 
              className="rounded-xl bg-[#7c83ff] px-4 py-1.5 text-sm font-medium text-white transition hover:opacity-90 disabled:opacity-50" 
            > 
        ...
        <div className="flex flex-1 overflow-hidden"> 
          ...
          {/* Columna derecha: formulario del bloque seleccionado */} 
          <div className="flex-1 overflow-y-auto p-4"> 
            {selectedBlock ? ( 
              ...
              <div className="flex h-full flex-col items-center justify-center text-center text-sm text-gray-500"> 
                ...
                Seleccioná un bloque para editarlo, o agregá uno nuevo desde la lista. 
              </div> 
            )} 
          </div> 
        </div> 
      ...
      return ( 
        <div className="space-y-4"> 
          ...
          <p className="text-xs text-gray-500"> 
            Las habitaciones se toman automáticamente de los datos del hostel (espacios activos). 
          </p> 
        </div> 
      ); 
