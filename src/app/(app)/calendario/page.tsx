...
type BloqueoNode = { id: Id; data: Bloqueo };
...
export default function CalendarioPage() {
  ...
  const [bloqueos, setBloqueos] = useState<BloqueoNode[]>([]);
  ...
  useEffect(() => {
    ...
    const qBloqueos = query(bloqueosCollection(hostelId), orderBy("desde", "desc"));
    const unsubBloqueos = onSnapshot(
      qBloqueos,
      (snap) => {
        setError(null);
        setBloqueos(snap.docs.map((d) => ({ id: d.id, data: d.data() })));
      },
      (e) => setError(errorMessage(e, "Error leyendo bloqueos")),
    );
      ...
      >
        ...
        <div className="min-w-0">
          ...
          {spacePanelOpen && spaceDetail ? (
            <div
              className="
                absolute right-4 top-4 z-20 h-[calc(100%-2rem)] w-[420px]
            ...
            >
              ...
              <div className="h-[calc(100%-152px)] overflow-auto px-4 pb-4 pt-4">
                {spacePanelTab === "estado" ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <SecondaryButton
                        type="button"
                        onClick={() => setSpacePanelTab("reservas")}
                      >
                        + Reservar
                      </SecondaryButton>
